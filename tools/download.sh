...
case "$OSTYPE" in
    linux*)
        ...
        case "$ID" in
            centos*)
                ...
                package=powershell-6.0.0_alpha.12-1.el7.centos.x86_64.rpm
                ;;
            ubuntu)
                if ! hash curl 2>/dev/null; then
                    echo "curl not found, installing..."
                ...
                case "$VERSION_ID" in
                    14.04)
                        package=powershell_6.0.0-alpha.12-1ubuntu1.14.04.1_amd64.deb
                        ;;
                    16.04)
                        package=powershell_6.0.0-alpha.12-1ubuntu1.16.04.1_amd64.deb
                        ;;
                    *)
    ...
    darwin*)
        ...
        package=powershell-6.0.0-alpha.12.pkg
        ;;
    *)
