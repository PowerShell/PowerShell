    ...
    linux*)
        ...
        case "$ID" in
            centos*)
                ...
                package=powershell-6.0.0_beta.2-1.el7.x86_64.rpm
                ;;
            ubuntu)
                if ! hash curl 2>/dev/null; then
                    echo "curl not found, installing..."
                ...
                case "$VERSION_ID" in
                    14.04)
                        package=powershell_6.0.0-beta.2-1ubuntu1.14.04.1_amd64.deb
                        ;;
                    16.04)
                        package=powershell_6.0.0-beta.2-1ubuntu1.16.04.1_amd64.deb
                        ;;
                    *)
            ...
            opensuse)
                ...
                case "$VERSION_ID" in
                    42.1)
                        package=powershell-6.0.0_beta.2-1.suse.42.1.x86_64.rpm
                        ;;
                    *)
