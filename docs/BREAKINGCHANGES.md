...
A bug in previous versions of PowerShell would override `$ErrorActionPreference` when specifying `-Verbose` or `Debug`.
Now, `$ErrorActionPreference` is always respected.
...
Fix is to also see if wildcard is being suppressed.
...
Sets `-NoTypeInformation` as the default for `Export-Csv` and `ConvertTo-Csv`,
...
and provides a terminating error when both `-NoTypeInformation` and `-IncludeTypeInformation` are supplied.
...
An error was added when a user tries to use `-Credential` (legacy usage without `-Authentication`) or `-UseDefaultCredentials` over a non-HTTPS URI.
Users can bypass the error with `-AllowUnencryptedAuthentication`.
