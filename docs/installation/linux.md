...
> Please note that we do not have an alpha.15 AppImage,
> this is preparation for the next release!

Using a recent Linux distribution,
download the AppImage `powershell-6.0.0-alpha.15-x86_64.AppImage`
from the [releases][] page onto the Linux machine.

Then execute the following in the terminal:

```
chmod a+x powershell-6.0.0-alpha.15-x86_64.AppImage
./powershell-6.0.0-alpha.15-x86_64.AppImage
```
