...
'@ -f $randomClassName
    ...
    Describe "Type and Namespace aliases are not allowed in constrained language mode" -Tags 'Feature','RequireAdminOnWindows' {
        
        BeforeAll {
            $script = @'
using namespace SysIO = System.IO
using type MyCustomList = System.Collections.Generic.List[char]
$List = [MyCustomList]::new()
$List.Add([SysIO.Path]::AltDirectorySeparatorChar)
'@
            ...
            $untrustedScriptFile = Join-Path $modulePath "T1TypeAlias.ps1"
            $script | Out-File -FilePath $untrustedScriptFile

            $untrustedScriptModule = Join-Path $modulePath "T1TypeAlias.psm1"
            $script | Out-File -FilePath $untrustedScriptModule

            $trustedScriptFile = Join-Path $modulePath "T1TypeAlias_System32.ps1"
            $script | Out-File -FilePath $trustedScriptFile

            $trustedScriptModule = Join-Path $modulePath "T1TypeAlias_System32.psm1"
            $script | Out-File -FilePath $trustedScriptModule
        }

        AfterAll {

            Remove-Module -Name T1TypeAlias_System32 -Force -ErrorAction Ignore
            Remove-Module -Name T1TypeAlias -Force -ErrorAction Ignore
        }

        It "Verifies that type and namespace aliases cannot be created in script running under constrained language" {
            ...
            $expectedError.FullyQualifiedErrorId | Should -BeExactly "TypeAndNamespaceAliasNotAllowedInConstrainedLanguage,Microsoft.PowerShell.Commands.InvokeExpressionCommand"
        }

        It "Verifies that type and namespace aliases cannot be created in script files running under constrained language" {
            ...
            $expectedError.FullyQualifiedErrorId | Should -BeExactly "TypeAndNamespaceAliasNotAllowedInConstrainedLanguage"
        }

        It "Verifies that type and namespace aliases cannot be created in untrusted script modules running under constrained language" {
        ...
        It "Verifies that type and namespace aliases can be created in trusted script files running under constrained language" {
        ...
        It "Verifies that type and namespace aliases can be created in trusted script modules running under constrained language" {
