...
Describe "Json Bug fixes"  -Tags "Feature" {
    ...
    $testCases = @(
        @{
            Name = "ConvertTo-Json -Depth 101 throws ParameterValidationError when the user specifies a depth greater than 100."
            NumberOfElements = 10
            MaxDepth = 101
            FullyQualifiedErrorId = "ParameterValidationError,Microsoft.PowerShell.Commands.ConvertToJsonCommand"
            ShouldThrow = $true
        }
        @{
