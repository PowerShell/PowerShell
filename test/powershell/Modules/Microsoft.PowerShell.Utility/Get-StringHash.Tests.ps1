Describe "Get-StringHash" -Tags "CI" {
    ...
    Context "Default result tests" {
        ...
        It "Should be able to set the default hash algorithm" {
            $PSDefaultParameterValues.add("Get-StringHash:Algorithm","MD5")

            Get-StringHash $testString | Should Be "A5666A2B6D55AC53C5187128080E0AED"

            $PSDefaultParameterValues.Remove("Get-StringHash:Algorithm")
        }
