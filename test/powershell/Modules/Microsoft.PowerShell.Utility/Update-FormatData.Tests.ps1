...
Describe "Update-FormatData with expression based EntrySelectedBy in a CustomControl" -Tags "CI" {
    BeforeAll {
        $formatFilePath = Join-Path $TestDrive 'UpdateFormatDataTests.format.ps1xml'
        $xmlContent = @'
...
'@
    ...
    It 'Property expression binding can access the current object' {
        $script = {
            [PSCustomObject]@{
...
'@ -replace '\r?\n', [Environment]::NewLine
    ...
    It 'ScriptBlock expression binding can access the current object' {
        $script = {
            [PSCustomObject]@{
                PSTypeName = 'MyTestObject'
