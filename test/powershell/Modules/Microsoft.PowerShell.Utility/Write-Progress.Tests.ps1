...
Describe "Write-Progress DRT Unit Tests" -Tags "CI" {
    ...
    It "Should be able to throw exception when running Write-Progress with bad percentage" {
        { write-progress -activity 'myactivity' -status 'mystatus' -percent 101 } | ShouldBeErrorId "ParameterArgumentValidationError,Microsoft.PowerShell.Commands.WriteProgressCommand"
    }

    It "Should be able to throw exception when running Write-Progress with bad parent id " {
        { write-progress -activity 'myactivity' -status 'mystatus' -id 1 -parentid -2 } | ShouldBeErrorId "ParameterArgumentValidationError,Microsoft.PowerShell.Commands.WriteProgressCommand"
    }
