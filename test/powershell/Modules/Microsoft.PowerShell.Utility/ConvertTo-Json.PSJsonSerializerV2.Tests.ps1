...
Describe 'ConvertTo-Json with PSJsonSerializerV2' -Tags "CI" {
    Context "Default values and limits" {
        It "V2: Default depth should be 64" -Skip:(-not $isV2Enabled) {
        ...
        It "V2: Depth 1000 is allowed (maximum)" -Skip:(-not $isV2Enabled) {
            { ConvertTo-Json -InputObject @{a=1} -Depth 1000 } | Should -Not -Throw
        }

        It "V2: Depth over 1000 should throw" -Skip:(-not $isV2Enabled) {
            { ConvertTo-Json -InputObject @{a=1} -Depth 1001 } | Should -Throw
        }

        It "V2: Negative depth should throw" -Skip:(-not $isV2Enabled) {
            { ConvertTo-Json -InputObject @{a=1} -Depth -1 } | Should -Throw
            { ConvertTo-Json -InputObject @{a=1} -Depth -2 } | Should -Throw
        }

        It "Legacy: Depth over 100 should throw when V2 is disabled" -Skip:$isV2Enabled {
            { ConvertTo-Json -InputObject @{a=1} -Depth 101 } | Should -Throw
        }
    }

    Context "Depth exceeded warning" {
        It "V2: Should output warning when depth is exceeded" -Skip:(-not $isV2Enabled) {
            $a = @{ a = @{ b = @{ c = @{ d = 1 } } } }
        ...
        It "V2: Should convert deep objects to string when depth exceeded" -Skip:(-not $isV2Enabled) {
    ...
    Context "Non-string dictionary keys (Issue #5749)" {
        It "V2: Should serialize dictionary with integer keys" -Skip:(-not $isV2Enabled) {
            $dict = @{ 1 = "one"; 2 = "two" }
        ...
        It "V2: Should serialize Exception.Data with non-string keys" -Skip:(-not $isV2Enabled) {
    ...
    Context "JsonIgnoreAttribute and HiddenAttribute" {
        It "V2: Should not serialize hidden properties in PowerShell class" -Skip:(-not $isV2Enabled) {
    ...
    Context "Special types" {
        It "V2: Should serialize Uri correctly" -Skip:(-not $isV2Enabled) {
            $uri = [uri]"https://example.com/path"
            $json = $uri | ConvertTo-Json -Compress
            $json | Should -BeExactly '"https://example.com/path"'
        }

        It "V2: Should serialize Guid correctly" -Skip:(-not $isV2Enabled) {
        ...
        It "V2: Should serialize BigInteger correctly" -Skip:(-not $isV2Enabled) {
        ...
        It "V2: Should serialize enums as numbers by default" -Skip:(-not $isV2Enabled) {
            $json = [System.DayOfWeek]::Monday | ConvertTo-Json
            $json | Should -BeExactly '1'
        }

        It "V2: Should serialize enums as strings with -EnumsAsStrings" -Skip:(-not $isV2Enabled) {
    ...
    Context "Null handling" {
        It "V2: Should serialize null correctly" -Skip:(-not $isV2Enabled) {
            $null | ConvertTo-Json | Should -BeExactly 'null'
        }

        It "V2: Should serialize DBNull as null" -Skip:(-not $isV2Enabled) {
            [System.DBNull]::Value | ConvertTo-Json | Should -BeExactly 'null'
        }

        It "V2: Should serialize NullString as null" -Skip:(-not $isV2Enabled) {
            [NullString]::Value | ConvertTo-Json | Should -BeExactly 'null'
        }

        It "V2: Should handle ETS properties on DBNull" -Skip:(-not $isV2Enabled) {
            try {
    ...
    Context "Collections" {
        It "V2: Should serialize arrays correctly" -Skip:(-not $isV2Enabled) {
            $arr = @(1, 2, 3)
            $json = $arr | ConvertTo-Json -Compress
            $json | Should -BeExactly '[1,2,3]'
        }

        It "V2: Should serialize hashtable correctly" -Skip:(-not $isV2Enabled) {
            $hash = [ordered]@{ a = 1; b = 2 }
            $json = $hash | ConvertTo-Json -Compress
            $json | Should -BeExactly '{"a":1,"b":2}'
        }

        It "V2: Should serialize nested objects correctly" -Skip:(-not $isV2Enabled) {
            $obj = [pscustomobject]@{
                name = "test"
    ...
    Context "EscapeHandling" {
        It "V2: Should not escape by default" -Skip:(-not $isV2Enabled) {
            $json = @{ text = "<>&" } | ConvertTo-Json -Compress
            $json | Should -BeExactly '{"text":"<>&"}'
        }

        It "V2: Should escape HTML with -EscapeHandling EscapeHtml" -Skip:(-not $isV2Enabled) {
        ...
        It "V2: Should escape non-ASCII with -EscapeHandling EscapeNonAscii" -Skip:(-not $isV2Enabled) {
