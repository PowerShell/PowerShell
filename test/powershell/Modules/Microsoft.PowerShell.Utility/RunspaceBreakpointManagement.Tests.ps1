...
Describe 'Runspace Breakpoint Unit Tests - Feature-Enabled' -Tags 'CI' {
    ...
    BeforeAll {
        ...
        # Start a job; this will create a runspace we can manage breakpoints in
        ...
        # Wait for the child job that gets created to actually start running
        while ($job.ChildJobs.Count -eq 0 -or $job.ChildJobs[0].State -ne 'Running') {
            Start-Sleep -Milliseconds 250
        }
