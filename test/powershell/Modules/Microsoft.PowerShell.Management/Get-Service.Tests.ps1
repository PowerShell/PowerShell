Describe "Get-Service cmdlet tests" -Tags "CI" {
  ...
  It "Get-Service fails for non-existing service using '<script>'" -TestCases @(
    @{script={Get-Service -Name (new-guid) -ErrorAction Stop};ErrorId="NoServiceFoundForGivenName,Microsoft.PowerShell.Commands.GetServiceCommand"},
    @{script={Get-Service -DisplayName (new-guid) -ErrorAction Stop};ErrorId="NoServiceFoundForGivenDisplayName,Microsoft.PowerShell.Commands.GetServiceCommand"}
  ) {
