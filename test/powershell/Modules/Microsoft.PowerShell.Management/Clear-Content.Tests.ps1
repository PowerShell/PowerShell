  ...
  BeforeAll {
    ...
    Context "Clear-Content should actually clear content" {
        ...
        It "the '-Stream' dynamic parameter should not be visible to get-command in the function provider" {
            ...
            { Get-Command Clear-Content -Stream $streamName } | Should -Throw -ErrorId "NamedParameterNotFound,Microsoft.PowerShell.Commands.GetCommandCommand"
            Pop-Location
        }
    }
    ...
    Context "Proper errors should be delivered when bad locations are specified" {
        It "should throw `"Cannot bind argument to parameter 'Path'`" when -Path is `$null" {
            { Clear-Content -Path $null -ErrorAction Stop} | Should -Throw -ErrorId "ParameterArgumentValidationErrorNullNotAllowed,Microsoft.PowerShell.Commands.ClearContentCommand"
        }

        #[BugId(BugDatabase.WindowsOutOfBandReleases, 903880)]
        It "should throw `"Cannot bind argument to parameter 'Path'`" when -Path is `$()" {
            { Clear-Content -Path $() -ErrorAction Stop } | Should -Throw -ErrorId "ParameterArgumentValidationErrorNullNotAllowed,Microsoft.PowerShell.Commands.ClearContentCommand"
        }
        ...
        It "should throw 'PSNotSupportedException' when you clear-content to an unsupported provider" {
            ...
            { Clear-Content -Path function:$functionName -ErrorAction Stop } | Should -Throw -ErrorId "NotSupported,Microsoft.PowerShell.Commands.ClearContentCommand"
        }

        It "should throw FileNotFound error when referencing a non-existant file" {
            $badFile = "TestDrive:/badfilename.txt"
            { Clear-Content -Path $badFile -ErrorAction Stop } | Should -Throw -ErrorId "PathNotFound,Microsoft.PowerShell.Commands.ClearContentCommand"
        }

        It "should throw DriveNotFound error when referencing a non-existant drive" {
            $badDrive = "{0}:/file.txt" -f (Get-NonExistantDriveName)
            { Clear-Content -Path $badDrive -ErrorAction Stop } | Should -Throw -ErrorId "DriveNotFound,Microsoft.PowerShell.Commands.ClearContentCommand"
        }
        ...
        It "should throw ProviderNotFound error when referencing a non-existant provider" {
            ...
            { Clear-Content -Path $badProviderPath -ErrorAction Stop } | Should -Throw -ErrorId "ProviderNotFound,Microsoft.PowerShell.Commands.ClearContentCommand"
        }
    }
}
