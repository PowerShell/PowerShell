...
Describe "Get-EventLog cmdlet tests" -Tags @('CI', 'RequireAdminOnWindows') {
    ...
    It "should return an array of eventlogs objects when called with -AsString parameter" -Skip:($true) {
    ...
    It "should return a list of eventlog objects when called with -List parameter" -Skip:($true) {
    ...
    It "should be able to Get-EventLog -LogName Application -Newest 100" -Skip:($true) {
    ...
    It "should throw 'AmbiguousParameterSetException' when called with both -LogName and -List parameters" -Skip:($true) {
    ...
    It "should be able to Get-EventLog -LogName * with multiple matches" -Skip:($true) {
      { $result=Get-EventLog -LogName *  -ErrorAction Stop }  | Should -Not -Throw
      $result                                                 | Should -Not -BeNullOrEmpty
      $result                                                 | Should -BeExactly "Security"
      $result.Count                                           | Should -BeGreaterThan 3
    }
    It "should throw 'InvalidOperationException' when asked to get a log that does not exist" -Skip:($true) {
