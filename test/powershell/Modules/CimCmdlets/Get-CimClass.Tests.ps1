...
Describe 'Get-CimClass' -Tags "CI" {
    ...
    It 'can get class when namespace is specified' -Skip:(-not $IsWindows) {
        Get-CimClass -ClassName CIM_OperatingSystem -Namespace root/cimv2 | Should -Not -BeNullOrEmpty
    }

    It 'produces an error when a non-existent class is used' -Skip:(-not $IsWindows) {
    ...
    It 'produces an error when an improper namespace is used' -Skip:(-not $IsWindows) {
...
Describe 'Get-CimClass' -Tags @("Feature") {
    It 'can retrieve a class when a method is provided' -Skip:(-not $IsWindows) {
        Get-CimClass -MethodName Reboot | Should -Not -BeNullOrEmpty
    }

    It 'can retrieve class amended qualifiers' -Skip:(-not $IsWindows) {
