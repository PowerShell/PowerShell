    ...
    Context "Default configuration file locations" {
        ...
        BeforeAll {
            ...
            if ($IsWindows) {
                ...
                $expectedModule   = [IO.Path]::Combine($env:USERPROFILE, "Documents", "PowerShell", "Modules")
                $expectedProfile  = [io.path]::Combine($env:USERPROFILE, "Documents","PowerShell",$profileName)
