...
'@).InvokeAsync()
    ...
    Context 'Test programmatic Control+Break to enter the debugger' {
        BeforeAll {
            $debuggerStopHandler = {
                param($s, $e)
                $e.ResumeAction = [System.Management.Automation.DebuggerResumeAction]::Stop
            }
            ...
            function InvokeAsyncHelper {
                ...
                $host.Runspace.Debugger.add_DebuggerStop($debuggerStopHandler)
                try {
                    $r = $PowerShell.AddScript("@(1,2,3,4,5)${sbStub}").InvokeAsync()
                    ...
                    $host.Runspace.Debugger.remove_DebuggerStop($debuggerStopHandler)
                }
            }
