Describe 'Testing of MyInvocation' -Tags "CI" {
    ...
    Context 'Script block' {
        ...
        It 'Script block3' { $a.ToLower().IndexOf("myinvocation.tests.ps1") | Should BeGreaterThan -1 }

        $a = . {$MyInvocation.ScriptName}

        It 'Script block4' { $a.ToLower().IndexOf("myinvocation.tests.ps1") | Should BeGreaterThan -1 }
    }
}
