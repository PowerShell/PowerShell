...
'@
    ...
    It 'Allowed cmdlets checked properly' {

        $script:exception = $null
        & {
            trap {$script:exception = $_.Exception ; continue }
            iex 'data d { @{ x=$(get-command)} }'
        }
    ...
    It 'Check fallback to current directory plus -SupportedCommand parameter' {
        ...
        import-localizeddata local:mydata -uiculture fr-ca -filename I18n.Test_fallback.psd1 -SupportedCommand MyConvertFrom-StringData
