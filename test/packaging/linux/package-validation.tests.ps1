...
Describe "Linux Package Name Validation" {
    ...
    Context "RPM Package Names" {
        It "Should accept the Azure Linux 4 <Architecture> package name" -TestCases @(
            @{
                Architecture = "x86_64"
                Name = "powershell-preview-7.6.0_preview.6-1.azl4.x86_64.rpm"
            }
            @{
                Architecture = "aarch64"
                Name = "powershell-preview-7.6.0_preview.6-1.azl4.aarch64.rpm"
            }
        ) {
            param($Name)
        ...
        It "Should reject malformed RPM package name <Name>" -TestCases @(
            @{ Name = "prefix-powershell-preview-7.6.0_preview.6-1.azl4.x86_64.rpm" }
            @{ Name = "powershell-preview-7.6.0_preview.6-1Xazl4.x86_64.rpm" }
            @{ Name = "powershell-preview-7.6.0_preview.6-1.azl4.x86_64.rpm.backup" }
        ) {
            param($Name)
