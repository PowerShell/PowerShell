...
namespace Microsoft.PowerShell.Commands
{
    ...
    public abstract partial class WebRequestPSCmdlet : PSCmdlet
    {
        ...
        internal virtual void PrepareSession()
        {
            ...
            if (CertificateThumbprint != null)
            {
                ...
                var tbCollection = (X509Certificate2Collection)collection.Find(X509FindType.FindByThumbprint, CertificateThumbprint, false);
                if (tbCollection.Count == 0)
                {
                    var ex = new CryptographicException(WebCmdletStrings.ThumbprintNotFound);
                    throw ex;
                }

                foreach (X509Certificate2 tbCert in tbCollection)
                {
                    var certificate = (X509Certificate)tbCert;
    ...
    public abstract partial class WebRequestPSCmdlet : PSCmdlet
    {
                ...
                if (!string.IsNullOrWhiteSpace(entry.Value))
                {
                    ...
                    else
                    {
                        ...
                        catch (FormatException ex)
                        {
                            ...
                            var er = new ErrorRecord(outerEx, "WebCmdletContentTypeException", ErrorCategory.InvalidArgument, ContentType);
        ...
        protected override void ProcessRecord()
        {
                        ...
                        using (HttpRequestMessage request = GetRequest(uri))
                        {
                            ...
                            try
                            {
                                ...
                                if (ShouldCheckHttpStatus && !_isSuccess)
                                {
                                    ...
                                    var httpEx = new HttpResponseException(message, response);
                                    var er = new ErrorRecord(httpEx, "WebCmdletWebResponseException", ErrorCategory.InvalidOperation, request);
                                ...
                                if (WebSession.MaximumRedirection == 0) // Indicate "HttpClientHandler.AllowAutoRedirect == false"
                                {
                                    ...
                                    {
                                        var er = new ErrorRecord(new InvalidOperationException(), "MaximumRedirectExceeded", ErrorCategory.InvalidOperation, request);
                            ...
                            {
                                var er = new ErrorRecord(ex, "WebCmdletWebResponseException", ErrorCategory.InvalidOperation, request);
            ...
            {
                var er = new ErrorRecord(ex, "WebCmdletCertificateException", ErrorCategory.SecurityError, null);
                ThrowTerminatingError(er);
            }
            catch (NotSupportedException ex)
            {
                var er = new ErrorRecord(ex, "WebCmdletIEDomNotSupportedException", ErrorCategory.NotImplemented, null);
                ThrowTerminatingError(er);
            }
        }
        ...
        internal long SetRequestContent(HttpRequestMessage request, XmlNode xmlNode)
        {
            ...
            byte[] bytes = null;
            var doc = xmlNode as XmlDocument;
                ...
                foreach (string linkHeader in links)
                {
                    foreach (string link in linkHeader.Split(","))
                    {
                        ...
                        if (match.Success)
                        {
                            ...
                            {
                                var absoluteUri = new Uri(requestUri, url);
