...
namespace System.Management.Automation.Internal
{
    ...
    internal class PipelineProcessor : IDisposable
    {
        ...
        private void Log(string logElement, InvocationInfo invocation, PipelineExecutionStatus pipelineExecutionStatus)
        {
            ...
            if (this.LocalPipeline is not null)
            {
            ...
            if ((invocation is null) || string.IsNullOrEmpty(invocation.Line))
            {
                if (hostInterface is not null)
                {
        ...
        internal void LogToEventLog()
        {
            if (NeedToLog())
            {
                ...
                if (_commands is null || _commands.Count == 0 || _eventLogBuffer.Count == 0)
                    return;
        ...
        private bool NeedToLog()
        {
            ...
            foreach (CommandProcessorBase commandProcessor in _commands)
            {
                ...
                if (cmdRuntime is not null && cmdRuntime.LogPipelineExecutionDetail)
                    return true;
            }
        ...
        {
            if (pipelineProcessor is null) throw PSTraceSource.NewArgumentNullException(nameof(pipelineProcessor));
            if (_redirectionPipes is null)
        ...
        internal int AddCommand(CommandProcessorBase commandProcessor, int readFromCommand, bool readErrorQueue)
        {
            ...
            else
            {
                ...
                if (prevcommandProcessor is null || prevcommandProcessor.CommandRuntime is null)
                {
                    // "PipelineProcessor.AddCommand(): previous request object is null"
                    throw PSTraceSource.NewInvalidOperationException();
                }
                ...
                {
                    // "PipelineProcessor.AddCommand(): UpstreamPipe is null"
                    throw PSTraceSource.NewInvalidOperationException();
                }
                ...
                if (commandProcessor.CommandRuntime.MergeUnclaimedPreviousErrorResults)
                {
                    for (int i = 0; i < _commands.Count; i++)
                    {
                        ...
                        if (prevcommandProcessor.CommandRuntime.ErrorOutputPipe.DownstreamCmdlet is not null)
                            continue;
                        if (prevcommandProcessor.CommandRuntime.ErrorOutputPipe.ExternalWriter is not null)
                            continue;
        ...
        internal Array SynchronousExecuteEnumerate(object input)
        {
            ...
            try
            {
                ...
                catch (PipelineStoppedException)
                {
                    StopUpstreamCommandsException stopUpstreamCommandsException =
                        _firstTerminatingError is not null
                            ? _firstTerminatingError.SourceException as StopUpstreamCommandsException
                            : null;
                    if (stopUpstreamCommandsException is null)
                    {
                        throw;
                    }
                    else
            ...
            catch (System.Runtime.InteropServices.InvalidComObjectException comException)
            {
                ...
                // which occurred during pipeline execution, regardless
                // of whether other errors occurred afterward.
                if (_firstTerminatingError is not null)
                {
            ...
            Diagnostics.Assert(toRethrowInfo is not null, "Alternate protocol path failure");
            toRethrowInfo.Throw();
        ...
        private void DoCompleteCore(CommandProcessorBase commandRequestingUpstreamCommandsToStop)
        {
            ...
            if (lastCommandRuntime is not null)
            {
                ...
                if ((this.LocalPipeline is null) || (!this.LocalPipeline.IsNested))
                {
...
#pragma warning restore 56500
        ...
        private void Start(bool incomingStream)
        {
            ...
            if (_commands is null || 0 == _commands.Count)
            {
                throw PSTraceSource.NewInvalidOperationException(
            ...
            CommandProcessorBase firstcommandProcessor = _commands[0];
            if (firstcommandProcessor is null
                || firstcommandProcessor.CommandRuntime is null)
            {
                throw PSTraceSource.NewInvalidOperationException(
                    PipelineStrings.PipelineExecuteRequiresAtLeastOneCommand);
            }
            ...
            if (LastCommandProcessor is null
                || LastCommandProcessor.CommandRuntime is null)
            {
                // "PipelineProcessor.Start(): LastCommandProcessor is null"
                throw PSTraceSource.NewInvalidOperationException();
            }

            if (ExternalSuccessOutput is not null)
            {
            ...
            if (ExternalInput is null && !incomingStream)
            {
        ...
        {
            if (ExternalErrorOutput is not null)
            {
                for (int i = 0; i < _commands.Count; i++)
                {
                    CommandProcessorBase commandProcessor = _commands[i];
        ...
        private void SetupParameterVariables()
        {
            ...
            {
                CommandProcessorBase commandProcessor = _commands[i];
                if (commandProcessor is null || commandProcessor.CommandRuntime is null)
                {
                    // "null command " + i
                    throw PSTraceSource.NewInvalidOperationException();
                }
        ...
        private Array RetrieveResults()
        {
            ...
            if (!_linkedErrorOutput)
            {
                ...
                for (int i = 0; i < _commands.Count; i++)
                {
                    CommandProcessorBase commandProcessor = _commands[i];
                    if (commandProcessor is null
                        || commandProcessor.CommandRuntime is null)
                    {
                    ...
                    if (ErrorPipe.DownstreamCmdlet is null && !ErrorPipe.Empty)
                    {
                        // 2003/10/02-JonN
            ...
            if (LastCommandProcessor is null
                || LastCommandProcessor.CommandRuntime is null)
            {
                // "PipelineProcessor.RetrieveResults(): LastCommandProcessor is null"
                throw PSTraceSource.NewInvalidOperationException();
            }
        ...
        {
            Dbg.Assert(pipeToUse is not null, "Caller should verify pipeToUse is not null");
        ...
        internal void LinkPipelineErrorOutput(Pipe pipeToUse)
        {
            ...
            for (int i = 0; i < _commands.Count; i++)
            {
                ...
                if (commandProcessor.CommandRuntime.ErrorOutputPipe.DownstreamCmdlet is null)
                {
...
#pragma warning disable 56500
                        ...
                        catch (Exception e) // Catch-all OK, 3rd party callout.
                        {
                            ...
                            if (commandProcessor.Command is not null)
...
#pragma warning restore 56500
        ...
        internal bool RecordFailure(Exception e, InternalCommand command)
        {
            ...
            {
                if (_firstTerminatingError is null)
                {
                ...
                // firstTerminatingError is PipelineStoppedException.
                    ...
                    && command is not null && command.Context is not null)
                {
                    Exception ex = e;
        ...
        {
            get { return _localPipeline is not null && _localPipeline.IsStopping; }
        }
