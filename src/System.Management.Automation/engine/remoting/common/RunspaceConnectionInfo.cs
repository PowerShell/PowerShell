...
// ReSharper restore CheckNamespace
{
    ...
    public sealed class SSHConnectionInfo : RunspaceConnectionInfo
    {
            ...
            ref StreamReader standardError)
        {
            ...
            // Spawn the child via posix_spawn(3). It creates pipes for the requested redirects, maps
            // the pipe ends onto the appropriate stdin/stdout/stderr descriptors, and execs the target.
            // posix_spawn is used (rather than a hand-rolled fork/execve) because executing managed code
            // in a forked process is not safe (only the calling thread transfers, thread IDs aren't stable
            // across the fork, etc.). The SSH process is created in its own session (ownSession) so that
            // Ctrl+C (SIGINT) signals do not propagate from PowerShell to it and PSRP handles them instead.
            int childPid = Interop.Unix.SpawnProcess(
