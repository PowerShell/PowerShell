...
namespace System.Management.Automation
{
	...
	internal class DeserializationContext
	{
		...
		{
			set { maxAllowedMemory = value; }
			get { return maxAllowedMemory; }
		}
		...
		internal void LogExtraMemoryUsage(int amountOfExtraMemory)
		{
			...
			if (maxAllowedMemory.HasValue)
			{
				if (amountOfExtraMemory > (maxAllowedMemory.Value - totalDataProcessedSoFar))
				{
				    string message = StringUtil.Format(Serialization.DeserializationMemoryQuota, ((double) maxAllowedMemory.Value)/(1 << 20),
				                                       ConfigurationDataFromXML.MAXRCVDOBJSIZETOKEN_CamelCase,
				                                       ConfigurationDataFromXML.MAXRCVDCMDSIZETOKEN_CamelCase);
					throw new XmlException(message);
				}

				totalDataProcessedSoFar = totalDataProcessedSoFar + amountOfExtraMemory;
			}
		}

		private int totalDataProcessedSoFar;
		private Nullable<int> maxAllowedMemory;
	...
	internal class CimClassDeserializationCache<TKey>
	{
		...
		{
			CimClass cimClass;
			if (this._cimClassIdToClass.TryGetValue(key, out cimClass))
			{
				/* PRINTF DEBUG
	...
	internal class InternalSerializer
	{
		...
		internal InternalSerializer(XmlWriter writer, SerializationContext context)
		{
			...
			this.objectRefIdHandler = new ReferenceIdHandlerForSerializer<object>(objectRefIdDictionary);

			this.typeRefIdHandler = new ReferenceIdHandlerForSerializer<ConsolidatedString>(
		...
		{
			Dbg.Assert(this.depthBelowTopLevel == 0, "InternalSerializer.depthBelowTopLevel should be 0 at top-level");
		...
		)
		{
			...
			try
			{
				...
				Dbg.Assert(this.depthBelowTopLevel <= MaxDepthBelowTopLevel, "depthBelowTopLevel should be <= MaxDepthBelowTopLevel");
			...
			{
				this.depthBelowTopLevel--;
				Dbg.Assert(this.depthBelowTopLevel >= 0, "depthBelowTopLevel should be >= 0");
			}
		}
		...
		private bool HandleMaxDepth(object source, string streamName, string property)
		{
			if (this.depthBelowTopLevel == MaxDepthBelowTopLevel)
			{
				...
				PSEtwLog.LogAnalyticError(PSEventId.Serializer_MaxDepthWhenSerializing, PSOpcode.Exception,
					PSTask.Serialization, PSKeyword.Serializer, source.GetType().AssemblyQualifiedName, property, this.depthBelowTopLevel);
		...
		private static Lazy<CimSerializer> cimSerializer = new Lazy<CimSerializer>(CimSerializer.Create);
		...
		private void PrepareCimInstanceForSerialization(PSObject psObject, CimInstance cimInstance)
		{
			...
			for (CimClass cimClass = cimInstance.CimClass; cimClass != null; cimClass = cimClass.CimSuperClass)
			{
				...
				byte[] miXmlBytes = cimSerializer.Value.Serialize(cimClass, ClassSerializationOptions.None);
			...
			{
				this._context.cimClassSerializationIdCache.AddClassToCache(serializedClassId);
			}
		...
		)
		{
			...
			if (writeTypeNames)
			{
				...
				{
					string typeNameHierarchyReferenceId = this.typeRefIdHandler.GetRefId(typeNames);
					if (typeNameHierarchyReferenceId == null)
					{
						...
						string tnRefId = this.typeRefIdHandler.SetRefId(typeNames);
		...
		private bool CanUseDefaultRunspaceInThreadSafeManner
		{
			get
			{
				...
				{
                    canUseDefaultRunspaceInThreadSafeManner = Runspace.CanUseDefaultRunspace;
				}
		...
		private Collection<CollectionEntry<PSMemberInfo>> ExtendedMembersCollection
		{
			get
			{
				if (extendedMembersCollection == null)
				{
					extendedMembersCollection = PSObject.GetMemberCollection(PSMemberViewTypes.Extended, _typeTable);
				}
				return extendedMembersCollection;
			}
		}
				...
				if (typesPs1xmlDepth > 0)
				{
					if (typesPs1xmlDepth != depth)
					{
						PSEtwLog.LogAnalyticVerbose(
							...
							PSKeyword.Serializer | PSKeyword.UseAlwaysAnalytic,
							pso.InternalTypeNames.Key, depth, typesPs1xmlDepth, this.depthBelowTopLevel);
		...
		private static string EncodeString(string s, int indexOfFirstEncodableCharacter)
		{
			...
			for (int i = indexOfFirstEncodableCharacter; i < slen; ++i)
			{
				...
				else
				{
					...
					result[rlen + 2 + 3] = hexlookup[c & 0x0F];
					c >>= 4;
					result[rlen + 2 + 2] = hexlookup[c & 0x0F];
					c >>= 4;
					result[rlen + 2 + 1] = hexlookup[c & 0x0F];
					c >>= 4;
					result[rlen + 2 + 0] = hexlookup[c & 0x0F];
	...
	internal class InternalDeserializer
	{
		...
		private bool UnknownTagsAllowed
		{
			get
			{
				Dbg.Assert(this._version.Major <= 1, "Deserializer assumes clixml version is <= 1.1");
		...
		private bool DuplicateRefIdsAllowed
		{
			get
			{
				...
				return (this._version < boundaryVersion);
#else
		...
		internal void ValidateVersion(string version)
		{
			...
			{
				throw NewXmlException(Serialization.UnexpectedVersion, null, this._version.Major);
			}
		}
		...
		private object ReadOneDeserializedObject(out string streamName, out bool isKnownPrimitiveType)
		{
			...
			_trace.WriteLine("Processing start node {0}", _reader.LocalName);
			...
			if (IsNextElement(SerializationStrings.ReferenceTag))
			{
				...
				object duplicate = this.objectRefIdHandler.GetReferencedObject(refId);
				if (duplicate == null)
				{
			...
			if (pktInfo != null)
			{
				_trace.WriteLine("Primitive Knowntype Element {0}", pktInfo.ItemTag);
			...
			{
				_trace.WriteLine("PSObject Element");
				return ReadPSObject();
			}
			...
			_trace.TraceError("Invalid element {0} tag found", _reader.LocalName);
		...
		private static Lazy<CimDeserializer> cimDeserializer = new Lazy<CimDeserializer>(CimDeserializer.Create);
		...
		private CimClass RehydrateCimClass(PSPropertyInfo classMetadataProperty)
		{
			...
			foreach (var deserializedClass in deserializedClasses)
			{
				...
				currentClass = this._context.cimClassSerializationIdCache.GetCimClassFromCache(cimClassSerializationId);
				...
				try
				{
					currentClass = cimDeserializer.Value.DeserializeClass(
						miXmlBytes, 
						ref offset, 
						parentClass, 
			...
			{
				this._context.cimClassSerializationIdCache.AddCimClassToCache(cacheEntry.Key, cacheEntry.Value);
			}
		...
		private PSObject ReadPSObject()
		{
			...
			while (_reader.NodeType == XmlNodeType.Element)
			{
				...
				else
				{
					...
					{
						_trace.WriteLine("Found container node {0}", ct);
						baseObject = ReadKnownContainer(ct);
					}
					else if (IsNextElement(SerializationStrings.PSObjectTag))
					{
						_trace.WriteLine("Found PSObject node");
						baseObject = ReadOneObject();
					}
					else
					{
						//We have an unknwon tag
						_trace.WriteLine("Unknwon tag {0} encountered", _reader.LocalName);
		...
		private PSObject ReadAttributeAndCreatePSObject()
		{
			...
			if (refId != null)
			{
				_trace.WriteLine("Read PSObject with refId: {0}", refId);
				this.objectRefIdHandler.SetRefId(sh, refId, this.DuplicateRefIdsAllowed);
			}
			return sh;
		}
		...
		private void ReadTypeNames(PSObject dso)
		{
			...
			if (IsNextElement(SerializationStrings.TypeNamesTag))
			{
				...
				_trace.WriteLine("Processing TypeNamesTag with refId {0}", refId);
			...
			{
				string refId = _reader.GetAttribute(SerializationStrings.ReferenceIdAttribute);
				_trace.WriteLine("Processing TypeNamesReferenceTag with refId {0}", refId);
				if (refId == null)
				{
					throw NewXmlException(Serialization.AttributeExpected, null, SerializationStrings.ReferenceIdAttribute);
				}

				ConsolidatedString typeNames = this.typeRefIdHandler.GetReferencedObject(refId);
				if (typeNames == null)
				{
		...
		static private readonly XmlReaderSettings xmlReaderSettingsForCliXml = GetXmlReaderSettingsForCliXml();
		static internal XmlReaderSettings XmlReaderSettingsForCliXml
		{
			get { return xmlReaderSettingsForCliXml; }
		}
		...
		static private readonly XmlReaderSettings xmlReaderSettingsForUntrustedXmlDocument = GetXmlReaderSettingsForUntrustedXmlDocument();
		static internal XmlReaderSettings XmlReaderSettingsForUntrustedXmlDocument
		{
			get { return xmlReaderSettingsForUntrustedXmlDocument; }
		}
		...
		private bool IsNextElement(string tag)
		{
			...
			return (_reader.LocalName == tag) &&
				((0 != (this._context.options & DeserializationOptions.NoNamespace)) ||
	...
	internal class ReferenceIdHandlerForSerializer<T> where T : class
	{
		...
		private readonly IDictionary<T, UInt64> object2refId;
		...
		internal string SetRefId(T t)
		{
			...
			{
				Dbg.Assert(!this.object2refId.ContainsKey(t), "SetRefId shouldn't be called when the object is already assigned a ref id");
				UInt64 refId = GetNewReferenceId();
				this.object2refId.Add(t, refId);
		...
		{
			UInt64 refId;
			if ((this.object2refId != null) && (this.object2refId.TryGetValue(t, out refId)))
			{
				return refId.ToString(System.Globalization.CultureInfo.InvariantCulture);
			}
			else
	...
	internal class ReferenceIdHandlerForDeserializer<T> where T : class
	{
		...
		internal void SetRefId(T o, string refId, bool duplicateRefIdsAllowed)
		{
			...
			{
				Dbg.Assert(!this.refId2object.ContainsKey(refId), "You can't change refId association");
			}
#endif
		...
		{
			Dbg.Assert(this.refId2object.ContainsKey(refId), "Reference id wasn't seen earlier");
			T t;
			if (this.refId2object.TryGetValue(refId, out t))
			{
				return t;
			}
			else
	...
	internal static class KnownTypes
	{
		...
		internal static TypeSerializationInfo GetTypeSerializationInfo(Type type)
		{
            ...
            if (!_knownTableKeyType.TryGetValue(type.FullName, out temp) && typeof(XmlDocument).IsAssignableFrom(type))
			{
				temp = _xdInfo;
			}
			return temp;
		...
		{
		    TypeSerializationInfo temp;
		    _knownTableKeyItemTag.TryGetValue(itemTag, out temp);
		    return temp;
		}
		...
		private static readonly TypeSerializationInfo _xdInfo =
	...
	internal class WeakReferenceDictionary<T> : IDictionary<object, T>
	{
...
#endif
		private int cleanupTriggerSize = initialCleanupTriggerSize;

		private void CleanUp()
		{
			if (this.Count > this.cleanupTriggerSize)
			{
				Dictionary<WeakReference, T> alive = new Dictionary<WeakReference, T>(this.weakEqualityComparer);
				foreach (KeyValuePair<WeakReference, T> weakKeyValuePair in this.dictionary)
				{
				...
				this.dictionary = alive;
				this.cleanupTriggerSize = initialCleanupTriggerSize + this.Count * 2;
			}
		}
		...
		public ICollection<object> Keys
		{
			get 
			{ 
				List<object> keys = new List<object>(this.dictionary.Keys.Count);
				foreach (WeakReference weakKey in this.dictionary.Keys)
				{
		...
		public bool TryGetValue(object key, out T value)
		{
			...
			return this.dictionary.TryGetValue(weakKey, out value);
		}
	...
	private static readonly Type[] handshakeFriendlyTypes = new Type[] {
		...
		private void VerifyValue(object value)
		{
			...
			foreach (Type goodType in handshakeFriendlyTypes)
			{
...
namespace Microsoft.PowerShell
{
	...
	public sealed class DeserializingTypeConverter : PSTypeConverter
	{
        ...
        private static readonly Dictionary<Type, Func<PSObject, object>> converter;

        static DeserializingTypeConverter()
        {
            converter = new Dictionary<Type, Func<PSObject, object>>();

            converter.Add(typeof(PSPrimitiveDictionary), RehydratePrimitiveHashtable);

            converter.Add(typeof(SwitchParameter), RehydrateSwitchParameter);
            converter.Add(typeof(PSListModifier), RehydratePSListModifier);
            converter.Add(typeof(PSCredential), RehydratePSCredential);
            converter.Add(typeof(PSSenderInfo), RehydratePSSenderInfo);
            converter.Add(typeof(CultureInfo), RehydrateCultureInfo);
            converter.Add(typeof(ParameterSetMetadata), RehydrateParameterSetMetadata);

            converter.Add(typeof(System.Security.Cryptography.X509Certificates.X509Certificate2), RehydrateX509Certificate2);
            converter.Add(typeof(System.Security.Cryptography.X509Certificates.X500DistinguishedName), RehydrateX500DistinguishedName);
            converter.Add(typeof(System.Net.IPAddress), RehydrateIPAddress);

            converter.Add(typeof(MailAddress), RehydrateMailAddress);

            converter.Add(typeof(System.Security.AccessControl.DirectorySecurity), RehydrateObjectSecurity<System.Security.AccessControl.DirectorySecurity>);
            converter.Add(typeof(System.Security.AccessControl.FileSecurity), RehydrateObjectSecurity<System.Security.AccessControl.FileSecurity>);
            converter.Add(typeof(System.Security.AccessControl.RegistrySecurity), RehydrateObjectSecurity<System.Security.AccessControl.RegistrySecurity>);

            converter.Add(typeof(ExtendedTypeDefinition), RehydrateExtendedTypeDefinition);
            converter.Add(typeof(FormatViewDefinition), RehydrateFormatViewDefinition);
            converter.Add(typeof(PSControl), RehydratePSControl);
            converter.Add(typeof(PSControlGroupBy), RehydrateGroupBy);
            converter.Add(typeof(DisplayEntry), RehydrateDisplayEntry);
            converter.Add(typeof(EntrySelectedBy), RehydrateEntrySelectedBy);
            converter.Add(typeof(TableControlColumnHeader), RehydrateTableControlColumnHeader);
            converter.Add(typeof(TableControlRow), RehydrateTableControlRow);
            converter.Add(typeof(TableControlColumn), RehydrateTableControlColumn);
            converter.Add(typeof(ListControlEntry), RehydrateListControlEntry);
            converter.Add(typeof(ListControlEntryItem), RehydrateListControlEntryItem);
            converter.Add(typeof(WideControlEntryItem), RehydrateWideControlEntryItem);
            converter.Add(typeof(CustomControlEntry), RehydrateCustomControlEntry);
            converter.Add(typeof(CustomItemBase), RehydrateCustomItemBase);

            converter.Add(typeof(CompletionResult), RehydrateCompletionResult);
            converter.Add(typeof(ModuleSpecification), RehydrateModuleSpecification);
            converter.Add(typeof(CommandCompletion), RehydrateCommandCompletion);
            converter.Add(typeof(JobStateInfo), RehydrateJobStateInfo);
            converter.Add(typeof(JobStateEventArgs), RehydrateJobStateEventArgs);
            converter.Add(typeof(PSSessionOption), RehydratePSSessionOption);
            converter.Add(typeof(LineBreakpoint), RehydrateLineBreakpoint);
            converter.Add(typeof(CommandBreakpoint), RehydrateCommandBreakpoint);
            converter.Add(typeof(VariableBreakpoint), RehydrateVariableBreakpoint);
            converter.Add(typeof(BreakpointUpdatedEventArgs), RehydrateBreakpointUpdatedEventArgs);
            converter.Add(typeof(DebuggerCommand), RehydrateDebuggerCommand);
            converter.Add(typeof(DebuggerCommandResults), RehydrateDebuggerCommandResults);
            converter.Add(typeof(DebuggerStopEventArgs), RehydrateDebuggerStopEventArgs);
        }
		...
		public override object ConvertFrom(PSObject sourceValue, Type destinationType, IFormatProvider formatProvider, bool ignoreCase)
		{
			...
			foreach (KeyValuePair<Type, Func<PSObject, object>> item in converter)
			{
				Type type = item.Key;
