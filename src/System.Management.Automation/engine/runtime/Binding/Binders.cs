...
namespace System.Management.Automation.Language
{
    ...
    internal sealed class PSBinaryOperationBinder : BinaryOperationBinder
    {
        ...
        private static bool IsSigned128BitCompatible(Type type)
        {
            return type.IsInt128Type() || LanguagePrimitives.IsSignedInteger(LanguagePrimitives.GetTypeCode(type));
        }

        private static DynamicMetaObject AddSignRestriction(DynamicMetaObject obj, bool isNegative)
        {
            Expression zero = obj.LimitType.IsInt128Type()
                ? Expression.Constant(Int128.Zero)
                : TypedZero(LanguagePrimitives.GetTypeCode(obj.LimitType));
            Expression value = obj.Expression.Cast(obj.LimitType);

            return new DynamicMetaObject(
                obj.Expression,
                obj.PSGetTypeRestriction()
                    .Merge(BindingRestrictions.GetExpressionRestriction(
                        isNegative ? Expression.LessThan(value, zero) : Expression.GreaterThanOrEqual(value, zero))),
                obj.Value);
        }

        /// <summary>
        /// Figures out the operation implementation when either operand is [Int128] or [UInt128].
        /// Those types are not primitive and their <see cref="TypeCode"/> is <see cref="TypeCode.Object"/>,
        /// so they cannot take part in the type code based promotion used for the other numeric types.
        /// </summary>
        /// <returns>True when the operation was figured out here, false to fall back to type code promotion.</returns>
        private static bool FigureInt128Op(ref DynamicMetaObject target, ref DynamicMetaObject arg,
                                          out Type opImplType, out Type argType)
        {
            opImplType = null;
            argType = null;

            if (!(target.LimitType.IsInt128Type() || target.LimitType.IsUInt128Type()
                  || arg.LimitType.IsInt128Type() || arg.LimitType.IsUInt128Type()))
            {
                return false;
            }

            // Enums and non-integer operands keep the existing promotion rules, which widen to the
            // underlying enum type, to [decimal] or to [double].
            if (target.LimitType.IsEnum || arg.LimitType.IsEnum
                || !target.LimitType.IsInteger() || !arg.LimitType.IsInteger())
            {
                return false;
            }

            if (target.LimitType.IsUInt128Type() || arg.LimitType.IsUInt128Type())
            {
                // [UInt128] can only stay unsigned while a signed operand is not negative, so the sign
                // is tested at bind time and captured as a restriction on the generated rule.
                bool signedIsTarget = IsSigned128BitCompatible(target.LimitType);
                if (signedIsTarget || IsSigned128BitCompatible(arg.LimitType))
                {
                    DynamicMetaObject signedOperand = signedIsTarget ? target : arg;
                    bool isNegative = signedOperand.LimitType.IsInt128Type()
                        ? (Int128)signedOperand.Value < Int128.Zero
                        : IsValueNegative(signedOperand.Value, LanguagePrimitives.GetTypeCode(signedOperand.LimitType));

                    DynamicMetaObject restricted = AddSignRestriction(signedOperand, isNegative);
                    if (signedIsTarget)
                    {
                        target = restricted;
                    }
                    ...
                    if (isNegative)
                    {
                        // [UInt128]::MaxValue is outside the range of both [Int128] and [decimal], so a
                        // negative operand leaves [double] as the only context wide enough for the operation.
            ...
            // Every remaining integer type, [UInt64] included, fits in [Int128].
        ...
        private DynamicMetaObject BinaryNumericOp(string methodName, DynamicMetaObject target, DynamicMetaObject arg)
        {
            ...
            if (FigureInt128Op(ref target, ref arg, out opImplType, out argType))
            {
                // The operation was figured out from the operand types rather than their type codes.
            }
            else if ((int)opTypeCode <= (int)TypeCode.Int32)
            {
