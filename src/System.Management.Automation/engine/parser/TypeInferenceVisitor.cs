...
namespace System.Management.Automation
{
    ...
    internal class TypeInferenceVisitor : ICustomAstVisitor2
    {
        ...
        private void InferTypesFrom(CommandAst commandAst, List<PSTypeName> inferredTypes)
        {
            ...
            {
                if (Activator.CreateInstance(inferenceType) is IOutputTypeProvider outputTypeProvider)
                {
                    var previousPipelineElement = GetPreviousPipelineCommand(commandAst);
                    var inputTypes = previousPipelineElement != null ? InferTypes(previousPipelineElement).ToArray() : PSTypeName.EmptyArray;
                    inferredTypes.AddRange(outputTypeProvider.GetOutputTypes(commandAst, inputTypes));
                }
            }

            var inferenceScript = commandInfo.OutputTypeProviderScriptBlock;
            if (inferenceScript != null)
            {
                var previousPipelineElement = GetPreviousPipelineCommand(commandAst);
                var inputTypes = previousPipelineElement != null ? InferTypes(previousPipelineElement).ToArray() : PSTypeName.EmptyArray;
                var result = inferenceScript.Invoke(commandAst, inputTypes);
                foreach (var res in result)
                {
                    switch (res.BaseObject)
                    {
                        case Type type:
                        {
