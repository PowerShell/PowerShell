...
namespace System.Management.Automation
{
    ...
    public sealed class ValidateSetAttribute : ValidateEnumeratedArgumentsAttribute
    {
        ...
        // The valid values cache works across 'ValidateSetAttribute' instances.
        // By default 'CacheExpiration = 0' and no valid values is cached (is re-evaluated every time it is used).
        private static ConcurrentDictionary<Type, ValidValuesCacheEntry> validValuesCache = new ConcurrentDictionary<Type, ValidValuesCacheEntry>();
        private class ValidValuesCacheEntry
        {
            public IValidateSetValuesGenerator validValuesGenerator = null;
            public string[] validValues;
            public DateTime cacheEntryCreationTime = DateTime.MinValue;

            public ValidValuesCacheEntry(IValidateSetValuesGenerator validValuesGenerator, string[] validValues, DateTime cacheEntryCreationTime)
            {
                this.validValuesGenerator = validValuesGenerator;
                this.validValues = validValues;
                this.cacheEntryCreationTime = cacheEntryCreationTime;
            }
        }
        ...
        public IList<string> ValidValues
        {
            get
            {
                ...
                {
                    ValidValuesCacheEntry validValuesCacheEntry;
                    if (validValuesCache.TryGetValue(validValuesGeneratorType, out validValuesCacheEntry))
                    {
                        var currentTime = DateTime.Now;
                        if (DateTime.Compare(validValuesCacheEntry.cacheEntryCreationTime.AddSeconds(CacheExpiration), currentTime) < 0)
                        {
                            // Update expired Entry.
                            validValuesCacheEntry.validValues = validValuesCacheEntry.validValuesGenerator.GetValidValues()?.ToArray();
                            validValuesCacheEntry.cacheEntryCreationTime = currentTime;
                            validValuesCache[validValuesGeneratorType] = validValuesCacheEntry;
                        }
                    }
                    else
                    {
                        var validValuesGenerator = (IValidateSetValuesGenerator)Activator.CreateInstance(validValuesGeneratorType);
                        var validValues = validValuesGenerator.GetValidValues()?.ToArray();
                        var cacheEntryCreationTime = DateTime.Now;
                        validValuesCacheEntry = new ValidValuesCacheEntry(validValuesGenerator, validValues, cacheEntryCreationTime);
                        validValuesCache.TryAdd(validValuesGeneratorType, validValuesCacheEntry);
                    }
                    if (validValuesCacheEntry.validValues == null)
                    {
                        throw new ValidationMetadataException(
