...
namespace System.Management.Automation
{
    ...
    internal abstract class Adapter
    {
            ...
            IFormatProvider formatProvider)
        {
            using (PSObject.MemberResolution.TraceScope("Method argument conversion."))
            {
                ...
                valueToConvert = UnReference(valueToConvert, out bool isArgumentByRef);
    ...
    internal class CacheTable
    {
        ...
        internal object this[string name]
        {
            get
            {
                if (!_indexes.TryGetValue(name, out int indexObj))
                {
                    return null;
                }
    ...
    internal class DotNetAdapter : Adapter
    {
        ...
        internal static bool DisallowPrivateReflection(Type type)
        {
            ...
            if (s_disallowReflectionCache.TryGetValue(assembly.FullName, out bool disallowReflection))
            {
        ...
        private static CacheTable GetStaticPropertyReflectionTable(Type type)
        {
            ...
            {
                if (s_staticPropertyCacheTable.TryGetValue(type, out CacheTable typeTable))
                {
                    return typeTable;
                }
        ...
        private static CacheTable GetStaticMethodReflectionTable(Type type)
        {
            ...
            {
                if (s_staticMethodCacheTable.TryGetValue(type, out CacheTable typeTable))
                {
                    return typeTable;
                }
        ...
        private static Dictionary<string, EventCacheEntry> GetStaticEventReflectionTable(Type type)
        {
            ...
            {
                if (s_staticEventCacheTable.TryGetValue(type, out Dictionary<string, EventCacheEntry> typeTable))
                {
                    return typeTable;
                }
        ...
        private static CacheTable GetInstancePropertyReflectionTable(Type type)
        {
            ...
            {
                if (s_instancePropertyCacheTable.TryGetValue(type, out CacheTable typeTable))
                {
                    return typeTable;
                }
        ...
        private static CacheTable GetInstanceMethodReflectionTable(Type type)
        {
            ...
            {
                if (s_instanceMethodCacheTable.TryGetValue(type, out CacheTable typeTable))
                {
                    return typeTable;
                }
        ...
        private static Dictionary<string, EventCacheEntry> GetInstanceEventReflectionTable(Type type)
        {
            ...
            {
                if (s_instanceEventCacheTable.TryGetValue(type, out Dictionary<string, EventCacheEntry> typeTable))
                {
                    return typeTable;
                }
...
#pragma warning restore 56500
        ...
        internal static object MethodInvokeDotNet(
            ...
            object[] arguments)
        {
            MethodInformation bestMethod = GetBestMethodAndArguments(methodName, methodInformation, invocationConstraints, arguments, out object[] newArguments);
        ...
        internal static object ConstructorInvokeDotNet(Type type, ConstructorInfo[] constructors, object[] arguments)
        {
            ...
            MethodInformation bestMethod = GetBestMethodAndArguments(type.Name, newConstructors, arguments, out object[] newArguments);
            return InvokeResolvedConstructor(bestMethod, newArguments, arguments);
        }
        ...
        internal static void ParameterizedPropertyInvokeSet(string propertyName, object target, object valuetoSet, MethodInformation[] methodInformation, object[] arguments)
        {
            ...
            MethodInformation bestMethod = GetBestMethodAndArguments(propertyName, methodInformation, arguments, out object[] newArguments);
    ...
    internal class XmlNodeAdapter : PropertyOnlyAdapter
    {
        ...
        protected override void DoAddAllProperties<T>(object obj, PSMemberInfoInternalCollection<T> members)
        {
            ...
            if (node.Attributes != null)
            {
                ...
                {
                    if (!nodeArrays.TryGetValue(attribute.LocalName, out List<XmlNode> nodeList))
                    {
            ...
            if (node.ChildNodes != null)
            {
                foreach (XmlNode childNode in node.ChildNodes)
                {
                    ...
                    if (!nodeArrays.TryGetValue(childNode.LocalName, out List<XmlNode> nodeList))
                    {
                        nodeList = new List<XmlNode>();
