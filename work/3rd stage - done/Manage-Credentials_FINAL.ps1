...
(
    ...
    [Parameter(Mandatory = $false)][ValidateSet("GENERIC",
        ...
        "DOMAIN_EXTENDED",
        "MAXIMUM",
        "MAXIMUM_EX")][SecureString] $CredType = "GENERIC",
    [Parameter(Mandatory = $false)][ValidateSet("SESSION",
        "LOCAL_MACHINE",
        "ENTERPRISE")][SecureString] $CredPersist = "ENTERPRISE"
)
...
function Get-CredType {
    ...
    (
        [Parameter(Mandatory = $true)][ValidateSet("GENERIC",
            ...
            "DOMAIN_EXTENDED",
            "MAXIMUM",
            "MAXIMUM_EX")][SecureString] $CredType
    )
...
function Get-CredPersist {
    ...
    (
        [Parameter(Mandatory = $true)][ValidateSet("SESSION",
            "LOCAL_MACHINE",
            "ENTERPRISE")][SecureString] $CredPersist
    )
...
#>
    ...
    (
        ...
        [Parameter(Mandatory = $true)][ValidateLength(1, 512)][SecureString] $Password,
